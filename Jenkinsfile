        ...
        stage('Deploy') {
            steps {
                withCredentials([[$class: 'UsernamePasswordMultiBinding',
                    ...
                    sh "vault auth -method=ldap username=$VAULT_USER password=$VAULT_PASSWORD && VAULT_TOKEN=\$(vault read -field id auth/token/lookup-self) make GIT_BRANCH=${env.BRANCH_NAME} PROVISIONER=${params.PROVISIONER} deploy"
                }
                sh 'sleep 999999'
            }
        }
